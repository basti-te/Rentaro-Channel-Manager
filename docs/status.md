...
| Full sync (hotel go-live / recovery) | `channex-full-sync` — 500-day availability + restrictions per property in exactly 2 calls; returns Channex task ids |
...
| Channex Full Sync | Apartments page: per-apartment **Full Sync** button + **„Alle synchronisieren"**. Pushes the complete **500-day** availability + rates/restrictions for a property in **exactly 2 Channex calls** (1× `/availability`, 1× `/restrictions`) — the certification "go-live" / recovery flow. Inngest function `channex-full-sync` (event `channex/full-sync`, throttle 4/min so a "sync all" paces under 20 ARI/min; reuses the `ari-resolve` span-compaction). `availability.push` / `restrictions.push` now parse + return the Channex **task ids**; the function records them on a `full_sync` `sync_jobs` row, the UI surfaces them (click-to-copy) for the cert submission. tRPC: `sync.fullSync` / `sync.fullSyncAll` / `sync.fullSyncStatus`. **Verified (sandbox E2E):** Whg 0, 500 days → 2 calls, task ids returned for both. |
...
│       └── src/inngest/        client, events.ts, functions/ (ari-flush, ari-resolve, ingest-bookings, channex-booking-mapper, channex-full-sync, messages-dispatch, cleaning-dispatch, stripe-event, billing-reconcile); webhooks/ (channex, twilio, stripe)
├── packages/
